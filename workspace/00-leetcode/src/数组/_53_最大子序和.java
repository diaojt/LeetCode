 ...
 * 该算法可看成动态规划的一个例子。   
 * 状态转移方程：sum[i] = max{sum[i-1]+a[i],a[i]} ---- sum[i]表示以a[i]为末端的最大子序列连续和
 * 其中(sum[i]记录以a[i]为子序列末端的最大序子列连续和)----这里很重要，a[i]为末端才能保证上边sum[i-1]+a[i]连续   
 * */
...
public class _53_最大子序和 {
	...
	public int maxSubArray(int[] nums) {
		...
		// 存放以nums[i]为末端的子序列的最大值
		int max_ending_here = nums[0];
		
		// 后边会分别得到以nums[1]为末端的子序列的最大值max_so_far1、以nums[2]为末端的子序列的最大值max_so_far2...
		// max_so_far存放着 max{max_so_far1,max_so_far2,....}
		int max_so_far = nums[0];
		
		for (int i = 1; i < nums.length; i++) {
			// 以每个位置为终点的最大子数列 都是基于其前一位置为末端的最大子序列计算得出
			// 比如以nums[1]为末端的最大子数列最大值，要么是nums[1]，要么是nums[1]+max_ending_here,
			// 其中max_ending_here中存放的是以nums[0]为末端的子序列的最大值
			max_ending_here = Math.max(nums[i] + max_ending_here, nums[i]);
			// 分别以nums[i]为末端的子序列最大值中最大的那个
